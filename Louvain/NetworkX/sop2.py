...
def ring_of_cliques_layout(clique_nodes, ring_radius=8.0, clique_radius=0.9):
    """Place clique centers on a single ring and orient bridge nodes to neighbors.

    This avoids spring-layout artifacts (e.g., figure-8 foldovers) and keeps
    the benchmark as a clean ring of cliques.
    """
    positions = {}
    n_cliques = len(clique_nodes)

    for i, nodes in enumerate(clique_nodes):
        theta = 2.0 * math.pi * i / n_cliques
        ...
        # Node 0 faces previous clique; last node faces next clique.
        first_angle = theta - (math.pi / n_cliques)
        last_angle = theta + (math.pi / n_cliques)
        positions[nodes[0]] = (
            cx + clique_radius * math.cos(first_angle),
            cy + clique_radius * math.sin(first_angle),
        )
        positions[nodes[-1]] = (
            cx + clique_radius * math.cos(last_angle),
            cy + clique_radius * math.sin(last_angle),
        )

        # Spread any remaining nodes to the back side of each clique.
        middle = nodes[1:-1]
        middle_count = len(middle)
        for j, node in enumerate(middle):
            ang = theta + math.pi + (2.0 * math.pi * j / max(1, middle_count))
            positions[node] = (
                cx + clique_radius * math.cos(ang),
                cy + clique_radius * math.sin(ang),
            )
...
pos = ring_of_cliques_layout(clique_groups)
n = G_ring.number_of_nodes()

internal_edges = []
bridge_edges = []

# Build exact bridge edge set from clique construction rule.
bridge_set = set()
for i in range(NUM_CLIQUES):
    u = clique_groups[i][-1]
    v = clique_groups[(i + 1) % NUM_CLIQUES][0]
    bridge_set.add(tuple(sorted((u, v))))

for u, v in G_ring.edges():
...
for ax, partition, title in [
    ...
    nx.draw_networkx_edges(
        G_ring, pos=pos, ax=ax, edgelist=internal_edges,
        edge_color="#9a9a9a", width=1.0, alpha=0.9,
    )
    nx.draw_networkx_edges(
        G_ring, pos=pos, ax=ax, edgelist=bridge_edges,
        edge_color="#444444", width=2.0,
    )
    nx.draw_networkx_nodes(
        G_ring, pos=pos, ax=ax, node_color=colors,
        node_size=350, edgecolors="#222222", linewidths=0.5,
    )
    nx.draw_networkx_labels(
        G_ring, pos=pos, ax=ax, font_size=7,
    )
